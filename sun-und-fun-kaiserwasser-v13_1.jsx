...
/*
  ...
  zum Vergleichen. Der Umschalter oben rechts wird nach der Entscheidung
  entfernt. Admin-Passwort: 3246
*/

const ADMIN_PW = "3246";

const THEMES = [
  { id: "a", name: "Seeglas", swatch: "#127c84" },
  { id: "b", name: "Sonnengruß", swatch: "#dd9026" },
  { id: "c", name: "Abendsteg", swatch: "#e8654e" },
];
    ...
    <div className="hotels">
      ...
      <div className="cards">
        ...
        <div className="card hotel">
          ...
          <p className="note">
            Preise pro Zimmer, ohne Frühstück. Hier kannst du direkt
            reservieren:
          </p>
                  ...
                  {COUNTED_SPORTS.map((s) => (
                    <div className="sportcount-row" key={s.id}>
                      <span className="sportcount-label">
                        {s.label.replace(" (ab 12 Anmeldungen)", "")}
                      </span>
...
function AdminDashboard({ settings, setSettings, onBack }) {
  ...
  const stats = useMemo(() => {
    ...
    yes.forEach((r) => {
      if (r.arrival) perArrival[r.arrival] = (perArrival[r.arrival] || 0) + 1;
    });
...
// =====================================================
//  STYLES  — klar & modern, viel Weiß, 3 dezente Varianten
// =====================================================
const CSS = `
...
.theme-switch-label{font-size:11px;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.08em;margin:0 2px}
.theme-dot{position:relative;width:22px;height:22px;border-radius:50%;border:2px solid var(--surface);outline:1px solid var(--line-2);background:var(--dot);cursor:pointer;padding:0;transition:transform .15s ease}
.theme-dot:hover{transform:scale(1.12)}
.theme-dot.on{outline:2px solid var(--ink)}
.theme-dot-name{position:absolute;top:130%;right:0;background:var(--ink);color:var(--bg);font-size:11px;padding:3px 8px;border-radius:6px;white-space:nowrap;opacity:0;pointer-events:none;transition:opacity .15s ease}
.theme-dot:hover .theme-dot-name{opacity:1}
.theme-sep{width:1px;height:18px;background:var(--line-2);margin:0 1px}
