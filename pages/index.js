...
const SPORTS = [
  ...
  { id: "tretboot", label: "Tretboot" },
  { id: "fussball", label: "Fußballplatz (ab 10 Anmeldungen)" },
];

// Fasst die Sport-Eintragungen einer Anmeldung als Text zusammen (für Tabelle/Export)
function sportSummary(r) {
  const parts = [];
  const sc = r.sportCounts || {};
  SPORTS.forEach((s) => {
    const n = Number(sc[s.id]) || 0;
    if (n > 0) parts.push(`${s.label.replace(" (ab 10 Anmeldungen)", "")} ${n}`);
  });
  return parts.join(", ") || "–";
}
...
const ARRIVAL = [
  ...
  { id: "ab17", label: "Ab 17:00 — Aperitif & Grillen" },
      ...
      <main className="container">
        ...
        <Section eyebrow="Aktiv sein" title="Sport & Wasser">
          ...
          <div className="sportlist">
            {SPORTS.map((s) => {
              const base = s.label.replace(" (ab 10 Anmeldungen)", "");
              const n = sportCounts[s.id] || 0;
              return (
                <div className="sportlist-row" key={s.id}>
                  <span>{base}</span>
                  <span className="chip-count">{s.id === "fussball" ? `${n}/10` : n}</span>
                </div>
              );
            })}
          </div>
          <p className="note">
            Ab 10 Mitspielern reserviere ich gerne ein kleines Fußballfeld
            für euch.
          </p>
        </Section>
        ...
        <Section eyebrow="Für den Tag" title="Was du mitbringen solltest">
          <div className="bring">
            {["Badesachen", "Handtuch", "Sportkleidung", "Sportschuhe", "Sonnencreme"].map((b) => (
              <div className="bring-item" key={b}>
                {b}
              </div>
            ))}
          </div>
        ...
        <Section eyebrow="Hinkommen" title="Anreise">
          <div className="cards">
            <div className="card">
              <h4>Öffentlich</h4>
              <p>
                U1 bis Station <strong>Alte Donau</strong>, dann ca. 5 Minuten zu
                Fuß zur Anlage.
              </p>
            </div>
            <div className="card">
              <h4>Mit dem Auto</h4>
              <p>
                Tiefgarage direkt am UniCredit Center (kostenpflichtig, nur
                begrenzt Plätze).
              </p>
              <p className="navi">
                Fürs Navi: <strong>Eiswerkstraße 20, 1220 Wien</strong>
              </p>
            </div>
          </div>
        </Section>
...
function RegistrationForm({ loaded, settings, onSubmitted }) {
  ...
  function validate() {
    ...
    if (f.attending === "yes") {
      const hatSport = Object.values(f.sportCounts || {}).some((n) => Number(n) > 0);
      if (!f.arrival) {
        e.arrival = "Wann kommst du?";
    ...
    if (Object.keys(e).length > 0) {
      setTimeout(() => {
        ...
        if (el) (el.closest(".field") || el).scrollIntoView({ behavior: "smooth", block: "center" });
      }, 60);
      return false;
    }
    return true;
    ...
    <div className="form" id="anmelden">
      <Field label="Kommst du?" error={errors.attending} required>
        <div className="seg">
          <button
            type="button"
            className={"seg-btn " + (f.attending === "yes" ? "on" : "")}
          ...
          <button
            type="button"
            className={"seg-btn " + (f.attending === "no" ? "on" : "")}
        ...
        <>
            ...
            <>
              <div className="grid2">
                <Field label="Partner/in dabei?">
                  <div className="seg">
                    <button type="button" className={"seg-btn " + (f.partner === "yes" ? "on" : "")} onClick={() => set("partner", "yes")}>
                      Ja
                    </button>
                    <button type="button" className={"seg-btn " + (f.partner === "no" ? "on" : "")} onClick={() => set("partner", "no")}>
                      Nein
                    </button>
                  </div>
                </Field>
                ...
                <Field label="Anzahl Kinder (bis 14 Jahre)">
                  <div className="stepper">
                    ...
                    <span>{f.kids}</span>
                    <button type="button" onClick={() => set("kids", Number(f.kids) + 1)}>
                      +
                    </button>
                  </div>
                </Field>
              ...
              <Field
                label="Wer macht welchen Sport mit?"
                hint="Anzahl Personen aus eurer Gruppe (inkl. dir, Partner, Kindern)"
              >
                <div className="sportcounts">
                  {SPORTS.map((s) => (
                    <div className="sportcount-row" key={s.id}>
                      <span className="sportcount-label">
                        {s.label.replace(" (ab 10 Anmeldungen)", "")}
                      </span>
                      <div className="stepper stepper-sm">
                        <button type="button" onClick={() => setSportCount(s.id, (f.sportCounts[s.id] || 0) - 1)}>
                          −
                        </button>
                ...
                <p className="note">
                  Alles freiwillig — du kannst auch einfach nur chillen, baden
                  und genießen.
                </p>
              </Field>
          ...
          <Field label={f.attending === "no" ? "Eine Nachricht (optional)" : "Noch etwas? (optional)"}>
            <textarea
              ...
              placeholder={f.attending === "no" ? "" : "Anmerkungen…"}
            />
          </Field>

          {Object.keys(errors).length > 0 && (
            <p className="form-error">Bitte fülle die markierten Pflichtfelder aus.</p>
          )}
...
function AdminDashboard({ settings, setSettings, onBack }) {
  ...
  // Gefilterte + sortierte Liste für die Tabelle (Statistik bleibt auf allen Daten)
  const shown = useMemo(() => {
  ...
  const stats = useMemo(() => {
    ...
    yes.forEach((r) => {
      ...
      SPORTS.forEach((s) => {
        ...
        if (n > 0) {
          perSport[s.id] += n;
          perSportNames[s.id].push(`${r.nachname || "?"} (${n})`);
        }
      });
    });
    const perArrival = {};
    ARRIVAL.forEach((a) => (perArrival[a.id] = { total: 0, adults: 0, kids: 0 }));
    yes.forEach((r) => {
      const slot = perArrival[r.arrival];
      if (slot) {
        const ad = 1 + (r.partner ? 1 : 0);
        const ki = Number(r.kids) || 0;
        slot.adults += ad;
        slot.kids += ki;
        slot.total += ad + ki;
      }
    });
    const vor20 = ["ab13", "ab17"].reduce(
      (sum, id) => {
        const p = perArrival[id] || { total: 0, adults: 0, kids: 0 };
        return { total: sum.total + p.total, adults: sum.adults + p.adults, kids: sum.kids + p.kids };
      },
      { total: 0, adults: 0, kids: 0 }
    );
    return { totalPeople, adults, children, perSport, perSportNames, perArrival, vor20 };
  }, [regs]);
        ...
        <>
          ...
          <div className="stat-cols">
            <div className="stat-col">
              <h4>Nach Ankunftszeit — Personen</h4>
              <table className="mini-tbl">
                ...
                <tbody>
                  {ARRIVAL.map((a) => {
                    const p = stats.perArrival[a.id] || { total: 0, adults: 0, kids: 0 };
                    return (
                      <tr key={a.id}>
                        <td>{a.label.split(" — ")[0]}</td>
                        <td>{p.total}</td>
                        <td>{p.adults}</td>
                        <td>{p.kids}</td>
                      </tr>
                  ...
                  <tr className="mini-tbl-sub">
                    <td>Vor 20:00 (Grillbuffet)</td>
                    <td>{stats.vor20.total}</td>
                    <td>{stats.vor20.adults}</td>
                    <td>{stats.vor20.kids}</td>
                  </tr>
                  <tr className="mini-tbl-sum">
                    <td>Summe</td>
            ...
            <div className="stat-col">
              <h4>Nach Sport</h4>
              {SPORTS.map((s) => (
                <div className="bar-block" key={s.id}>
                  <div className="bar-row">
                    <span>{s.label.replace(" (ab 10 Anmeldungen)", "")}</span>
                    <strong>{stats.perSport[s.id] || 0}</strong>
                  </div>
                  {stats.perSportNames[s.id] && stats.perSportNames[s.id].length > 0 && (
                    <p className="bar-names">{stats.perSportNames[s.id].join(", ")}</p>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="admin-actions">
            <button className="btn btn-accent" onClick={exportCSV} disabled={shown.length === 0}>
...
/* ---------- DARK MODE (neutrale Töne, Akzent bleibt je Variante) ---------- */
...
/* CHIPS / BRING */
.sportlist{display:flex;flex-direction:column;gap:8px;max-width:460px}
.sportlist-row{display:flex;align-items:center;justify-content:space-between;gap:12px;background:var(--bg-soft);border:1px solid var(--line);border-radius:12px;padding:11px 14px;font-weight:500;font-size:15px}
.chip-count{display:inline-flex;align-items:center;justify-content:center;min-width:22px;height:20px;padding:0 7px;border-radius:999px;background:var(--accent);color:var(--on-accent);font-size:12px;font-weight:700;line-height:1}
.bring{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px}
.bring-item{background:var(--bg-soft);border:1px solid var(--line);border-radius:12px;padding:12px 18px;font-weight:500;font-size:15px;text-align:center}
...
.bar-block{border-bottom:1px solid var(--line)}
.bar-block:last-child{border-bottom:none}
.bar-block .bar-row{border-bottom:none;padding-bottom:2px}
.bar-names{margin:0 0 10px;font-size:12.5px;color:var(--muted);line-height:1.5}
.mini-tbl{width:100%;border-collapse:collapse;font-size:14px}
.mini-tbl th{text-align:right;font-weight:600;color:var(--muted);font-size:12px;padding:0 0 8px;text-transform:uppercase;letter-spacing:.04em}
.mini-tbl th:first-child{text-align:left}
.mini-tbl td{padding:8px 0;border-top:1px solid var(--line);text-align:right}
.mini-tbl td:first-child{text-align:left}
.mini-tbl-sum td{font-weight:700;font-family:var(--font-d)}
